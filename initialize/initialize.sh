...
#1 Load $info_file 
#2 Check if run by root 
#3 Update and upgrade 
#4 Install neecssary softwares 
#5 Disable AppArmor 
#6 Disable UFW 
#7 Set hostname 
#8 Enable BBR
#9 Add SSH public keys 
#10 Modify SSH configuration  
#11 Enable firewalld 
#12 Set up swap file 
#13 Generate random root password 
#14 Add aliases (ll, ls)
...
setup_swapfile() {
    ...
    # $swapfile_size in MB
    dd if=/dev/zero of=$swapfile_location bs=1M count="$swapfile_size"
...
change_root_password() {
    ...
    echo -n "$root_password" | openssl passwd -stdin | usermod --password /dev/stdin root
    #echo "$root_password" > /root/root_password
}
...
ending_message() {
    echo -e "\n\nAll done!"
    #echo "Your New Root Password is saved in /root/root_password"
